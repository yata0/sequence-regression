...
def process_head_result(result_file,target_root,padding=True):
    dims = 54
    result = np.loadtxt(result_file)
    to_result = process_head(result,padding)
    basename = os.path.basename(result_file)
    target_name = os.path.join(target_root, basename)
...
def process_head(data,padding):
    """
    padding: head
    """
    dims = 54
    max_vector = np.loadtxt("./statistic/transform/max.txt")
    min_vector = np.loadtxt("./statistic/transform/min.txt")
    to_result = data * (max_vector - min_vector) + min_vector
    if padding:
        temp = np.zeros([data.shape[0],dims])
        temp[:,:3] = to_result
        to_result = temp
    return smooth(to_result/2)
...
def process_for_new(result_file, target_root="./lijianan"):
    """
    动作：不用padding
    """
    ...
    head = process_head(head,padding=False)
    head = list(head)
...
def process_for_new_siyuanshu(result_file, target_root="./lijianan"):
    """
    动作：不用padding
    四元数
    """
